...
do
        ...
        sudo curl  $SERVER/TEZ_TASK_ID?primaryFilter=TEZ_DAG_ID:$line > $PERFDATA_OUTPUTDIR/tasks/tasks_tpch_query_$count.txt
        echo "Getting task attempts data for $line"
        sudo curl  $SERVER/TEZ_TASK_ATTEMPT_ID?primaryFilter=TEZ_DAG_ID:$line > $PERFDATA_OUTPUTDIR/taskattempts/taskattempts_tpch_query_$count.txt
        ((count++))
done < $file
